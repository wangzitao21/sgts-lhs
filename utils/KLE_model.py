# -*- coding: utf-8 -*-
"""
Created on Wed Jan  1 13:38:43 2020
The KLE code is from ZNanzhe Wang's article "Deep-Learning-Based Inverse Modeling Approaches: A Subsurface Flow Example".
"""
import numpy as np
...
#    print(num)
    ...
    ax1.plot(range(num)[:80], (lamda_all/domain/var)[:80], color='#b86366', marker='o', linestyle='--', markersize=5, markeredgecolor='black', label='Labels')

#    ax1.set_xlim([1,n_eigen])
#    plt.legend()
    ax1.set_xlabel('$n$')
    ax1.set_ylabel('Lamda 2D / (Domain*Var)')  
    plt.title('Series of Engenvalues in 2 Dimensions')
    trans = mtransforms.ScaledTranslation(10/72, -5/72, fig.dpi_scale_trans)
    ax1.text(0.9, 0.9, "(a)", transform=ax1.transAxes + trans, verticalalignment='top')

    ax1.grid(visible=True, which='major', axis='both', alpha=0.5, linestyle='--')
    ax1.set_axisbelow(True)
    
    ax2 = fig.add_subplot(122)
    ax2.plot(range(num)[:80],(sum_lamda/domain/var)[:80], color='#5853bc', marker='o', linestyle='--', markersize=5, markeredgecolor='black', label='Labels')

    ax2.grid(visible=True, which='major', axis='both', alpha=0.5, linestyle='--')
    ax2.set_axisbelow(True)
    plt.axvline(x=7, ymax=0.40)
    plt.axvline(x=12, ymax=0.50)
    plt.axvline(x=34, ymax=0.70)
    plt.axvline(x=48, ymax=0.75)
    plt.axvline(x=71, ymax=0.80)
#    ax1.set_xlim([1,n_eigen])
#    plt.legend()
    ax2.set_xlabel('$n$')
    ...
    trans = mtransforms.ScaledTranslation(10/72, -5/72, fig.dpi_scale_trans)
    ax2.text(0.9, 0.9, "(b)", transform=ax2.transAxes + trans, verticalalignment='top')
